  ...
  <div class="max-w-xl mx-auto py-10 px-4">
    ...
    <form @submit.prevent="submitArt" class="space-y-4">
      <div>
        ...
        <input v-model="title" type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required />
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700">Categoria</label>
        <input v-model="category" type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required />
      </div>
      ...
      <button type="submit" class="px-4 py-2 bg-gray-800 text-white rounded hover:bg-gray-700">Salva Opera</button>
    </form>
  </div>
</template>
